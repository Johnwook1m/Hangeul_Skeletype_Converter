...
Glyphs 3용 플러그인: 한글 글리프의 중심선(centerline)을 자동으로 추출하여 
"SVG Import" 레이어에 생성합니다.
"""
...
class HanguelSkeletypeConverter(GeneralPlugin):
    """Glyphs 3 플러그인: 한글 글리프의 중심선 추출"""
    
    def settings(self):
        """플러그인 설정"""
        self.name = "Hanguel Skeletype Converter"
        self.keyboardShortcut = None  # 단축키 설정 (선택사항)
    ...
    def start(self):
        ...
        try:
            ...
            newMenuItem = NSMenuItem(self.name, self.run)
